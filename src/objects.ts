...
export function isCorrect(question: Question, answer: string): boolean {
    const exp = String(question.expected.toLowerCase()).trim();
...
export function isValid(question: Question, answer: string): boolean {
    ...
    const new_arr = question.options.filter(
...
export function toShortForm(question: Question): string {
    const new_name = question.name.slice(0, 10);
    const new_id = String(question.id);
...
export function mergeQuestion(
    ...
    const new_ques = { ...contentQuestion };
    new_ques.id = id;
    new_ques.name = name;
    new_ques.points = points;
    new_ques.published = false;
    return new_ques;
}
